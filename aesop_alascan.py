...
class Alascan:
    ...
    selstr : TYPE
        Description
    """
    def __init__(self, pdb, path_pdb2pqr, path_apbs, selstr=['protein'], jobname=None, region=None, grid=1, ion=0.150, pdie=20.0, sdie=78.54):
        self.pdb = pdb
...
def execPDB2PQR(path_pdb2pqr_exe, pdbfile, optargs='--ff charmm --chain', outfile=None):
    ...
    log = runProcess([path_pdb2pqr_exe, '--ff=charmm', '--chain', pdbfile, outfile])
    return log
